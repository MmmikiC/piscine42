/* ************************************************************************** */
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   ft_strncat_main.c                                  :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: pichen <pichen@student.42tokyo.jp>         +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/02/16 11:57:20 by pichen            #+#    #+#             */
/*   Updated: 2023/02/16 12:06:16 by pichen           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include <stdio.h>
#include <string.h>

char	*ft_strncat(char *dest, char *src, unsigned int nb)
{
	int				i;
	unsigned int	check_nb;

	i = 0;
	check_nb = 0;
	while (dest[i] != '\0')
		i++;
	while (check_nb < nb && *src != '\0')
	{
		dest[i] = *src;
		i++;
		src++;
		check_nb++;
	}
	dest[i] = '\0';
	return (dest);
}

int	main(void)
{
	char			dest[50] = "I'm in 42Tokyo ";
	char			src[50] = "and I feel sleepy...";
	char			dest1[50] = "I'm in 42Tokyo ";
	char			src1[50] = "and I feel sleepy...";
	unsigned int	nb;

	nb = 10;
	printf("strncat in string.h:\n");
	printf("dest: \"%s\", src: \"%s\", nb: \"%u\"\n", dest, src, nb);
	printf("After strncat: %s\n", strncat(dest, src, nb));
	printf("\n");
	printf("ft_strncat written by myself:\n");
	printf("dest1: \"%s\", src1: \"%s\", nb: \"%u\"\n", dest1, src1, nb);
	printf("After ft_strncat: %s\n", ft_strncat(dest1, src1, nb));

}
